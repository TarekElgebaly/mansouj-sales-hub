-- Shopify product, variant, inventory, location, and cost sync tables.
-- These tables store read-only Shopify Admin API data for later cost/profit backfill.
...
CREATE TRIGGER shopify_variants_updated_at
...
CREATE TRIGGER shopify_inventory_items_updated_at
  BEFORE UPDATE ON public.shopify_inventory_items
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS shopify_locations_updated_at ON public.shopify_locations;
CREATE TRIGGER shopify_locations_updated_at
...
CREATE TRIGGER shopify_inventory_levels_updated_at
...
CREATE POLICY "shopify products read ops finance"
...
CREATE POLICY "shopify variants read ops finance"
  ON public.shopify_variants
  FOR SELECT
  TO authenticated
  USING (public.can_ops(auth.uid()) OR public.has_role(auth.uid(), 'finance'));

DROP POLICY IF EXISTS "shopify inventory items read ops finance" ON public.shopify_inventory_items;
CREATE POLICY "shopify inventory items read ops finance"
  ON public.shopify_inventory_items
  FOR SELECT
  TO authenticated
  USING (public.can_ops(auth.uid()) OR public.has_role(auth.uid(), 'finance'));

DROP POLICY IF EXISTS "shopify locations read ops finance" ON public.shopify_locations;
CREATE POLICY "shopify locations read ops finance"
...
CREATE POLICY "shopify inventory levels read ops finance"
